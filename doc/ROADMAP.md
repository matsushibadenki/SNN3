...
* **主要技術（修正案）：**  
  * **1\. DigitalLifeForm オーケストレーターの高度化：**  
    * **現状の課題：** 「好奇心、退屈、自信」といった内部状態の定義が曖昧で、それに基づくい意思決定プロセスが不明確。  
    * **修正案：** IntrinsicMotivationSystemから受け取る内部状態を、以下のような**定量的な指標**に具体化する。  
      * **好奇心 (Curiosity):** 予測誤差の大きさや、GlobalWorkspaceにおける情報の不確実性（エントロピー）として定義。  
      * **自信 (Confidence):** 特定タスクにおける成功率や、予測の安定性として定義。  
      * **退屈 (Boredom):** 同一または類似タスクの継続時間や、学習進捗の停滞度（損失関数の変化率の低下）として定義。  
    * これらの指標をインプットとし、マスタープロセスは\*\*状態遷移モデル（例：有限状態マシン、確率的ポリシー）\*\*に基づいて、次に起動するエージェント（PlannerAgent, RLAgent, SelfEvolvingAgent）を決定する。これにより、曖昧な感情表現ではなく、明確なデータに基づいた意思決定が可能となる。  
  * **2\. 統一された長期記憶の構造化：**  
    * **現状の課題：** snn\_research/agent/memory.pyにおける経験の保存方法が非構造的で、ドメイン横断的な洞察を得るのが困難。  
    * **修正案：** 経験を単なるログとしてではなく、\*\*構造化されたデータ（経験タプル）\*\*としてagent\_memory.jsonlに記録する。  
      * **経験タプルの構造:** { "state": \<状態\>, "action": \<行動\>, "result": \<結果\>, "reward": \<報酬\>, "expert\_used": \[\<使用した専門家SNNのID\>\], "timestamp": \<タイムスタンプ\> }  
      * この構造化により、GlobalWorkspaceは過去の経験を効率的に検索・分析できるようになる。例えば、PlannerAgentが新しいタスクに直面した際、類似したstateを持つ過去の成功体験を検索し、expert\_usedやactionを参考にすることで、ゼロから計画を立てるよりも効率的に解にたどり着けるようになる。  
  * **3\. 文脈に応じた状態遷移ロジックの洗練：**  
    * **現状の課題：** 状態遷移のトリガー（例：「タスクの失敗」「性能の低さ」）が単純で、複雑な状況に対応できない。  
    * **修正案：** MetaCognitiveSNNを導入して、タスクの成否だけでなく、**学習プロセスのメタデータ**も監視・評価する。  
      * **監視対象メタデータ：**  
        * 学習の収束速度（損失関数の勾配）  
        * リソース消費量（計算時間、エネルギー効率）  
        * 専門家SNN間の情報フローのパターン  
      * このメタデータ分析に基づき、失敗の原因をより正確に診断し、適切な対応を決定する。  
        * **シナリオ例1（知識不足）：** 未知のデータ領域でのタスク失敗が続く場合（予測誤差が高い状態で安定）、AutonomousAgentを起動してWebからの情報収集を指示する。  
        * **シナリオ例2（能力不足）：** 特定のタスクの精度がプラトーに達した場合（学習の収束速度がゼロに近づく）、SelfEvolvingAgentを起動して、関連する専門家SNNのアーキテクチャ進化を促す。  
        * これにより、「なぜ失敗したのか」という文脈を考慮した、よりインテリジェントな自己改善ループが実現される。  
* **達成指標：** life-form startコマンド実行後、エージェントのログファイルが、長期間にわたり、多様な活動（例：Webクローリング、新しい専門家の訓練、それを用いた計画、性能評価、そして自己進化の誘発）の継続的で**指示のないシーケンス**を示す。さらに、ログには**状態遷移のトリガーとなった定量的指標（例：予測誤差、成功率）と、それに対応するオーケストレーターの意思決定**が明確に記録されている。
